...
- Minor refinement (filed as a candidate 4.7.1 patch): the script's awk regex for extracting the "Recommended reading order:" section exits early when there is a blank line between the header and the numbered list. Repos following the LLM-DocKit template extract correctly; repos that customise their LLM_START_HERE.md with a blank-line gap fall back to a generic 2-item list. Hook still fires and the Onboarding-loaded protocol still works — only the per-repo customisation is degraded. Surfaced during the 2026-05-03 smoke test in `home-infra-protocol`.
