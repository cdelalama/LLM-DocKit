...
- Session Focus: **Cut v4.9.6 to upstream youtube2text local-decision-019-style guardrails without breaking the mixed HISTORY-format fleet.** `history-entry` now accepts dash and no-dash dated entries by default (`history_format: any`) while allowing strict `dash` or `no-dash` through .dockit-config.yml; newest-first and "first real dated entry is today" remain enforced. Trace durable HISTORY scanning now handles both entry shapes. Version sync now has additive handlers for `json-version`, `yaml-info-version`, and `package-lock-version` in both check and bump scripts; unknown marker types now fail. DF-045 is filed and implemented; D-010 records the policy. `scripts/test-validator.sh` is expanded to 32 cases. Rollout is intentionally cautious: verify one dash-format adopter and one no-dash-format adopter before a broad sync; do not mark youtube2text local decision 019 closed until youtube2text re-syncs and removes or supersedes its local fork.
