...
- Session Focus: **Cut v4.9.1 (patch) hardening DF-040 Trace Protocol after auditor finding.** v4.9.0 added the synced `trace-protocol` section to `LLM_START_HERE.md`, extended `scripts/dockit-bootstrap-context.sh` so SessionStart payloads remind executor/auditor agents to begin substantive reports with a compact Trace header and then explain in prose, and added `check_trace_protocol` to `scripts/dockit-validate-session.sh`. Audit of `d6fc816` found a POSIX shell precedence bug in `_trace_validate_commit`: `! cd "$PROJECT_ROOT" && git cat-file ...` negated only `cd`, so invalid HANDOFF Trace Anchor hashes passed silently. v4.9.1 refactors the helper to `git -C "$PROJECT_ROOT"` throughout, adds invalid-hash regression coverage, and accepts HANDOFF commit times with or without seconds (`YYYY-MM-DD HH:MM:SS UTC` or `YYYY-MM-DD HH:MM UTC`). `scripts/test-validator.sh` now covers 17 cases. DF-040 status -> `implemented (4.9.0; hardened in 4.9.1)`. DF-035 option (b) remains the primary open candidate.
