...
- Status: The new proposal formalises the propose / critique / arbitrate pattern — already in informal use during the construction of operator's `tomatic` project — as a named primitive of LLM-DocKit. It defines roles, invocation thresholds, mechanics, a structured `REVIEWS.md` recording format, defined failure modes, and the explicit relationship with the existing `DOWNSTREAM_FEEDBACK.md` + `*_PROPOSAL.md` channels. The proposal is self-contained for a future implementing session. ForgeOS is mentioned as future consumer / precedent, NOT as design requirement (rejected during the run by GPT-5: "no convertir ForgeOS en requisito formal"). Cross-repo: sibling proposal `~/src/home-infra-protocol/docs/DEPLOYMENT_EVIDENCE_PROPOSAL.md` produced by the same consensus run; DF-029 in this repo moved to `partially accepted` with cross-reference. Adopter count: 29 entries (DF-029 added 2026-05-03).
- Pending Proposals: `docs/CONSENSUS_PROTOCOL_PROPOSAL.md` (this run); `docs/HOOKS_ENFORCEMENT_PROPOSAL.md` (untracked, pre-existing); `docs/LLM_DOCKIT_CE_V2_PROPOSAL.md` (untracked, pre-existing).
