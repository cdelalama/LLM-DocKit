...
**Two candidates, operator chooses scope at next session open:**
...
3. **DF-039 — validator escape for zero-diff sessions** (filed 2026-05-17, third recurrence in 9 days). `check_handoff_date` and `check_history_entry` should short-circuit on `git diff HEAD --quiet` (zero tracked-file diff = read-only session, nothing to document). Closes the recurring false-positive where `/brief` sessions are blocked at Stop and forced into bookkeeping mini-updates that themselves trigger the next session's block. Files-to-touch in `docs/DOWNSTREAM_FEEDBACK.md` DF-039 *Implementation hints*. Could ship as v4.8.1 alongside (2) (same file, same check pattern — bundle justified) or as v4.8.2 if (2) ships solo. Higher operator-friction impact than (2).
