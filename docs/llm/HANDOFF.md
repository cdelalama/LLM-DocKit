...
- Session Focus: **Cut v4.9.0 (minor) shipping DF-040 Trace Protocol.** Added the synced `trace-protocol` section to `LLM_START_HERE.md`, extended `scripts/dockit-bootstrap-context.sh` so SessionStart payloads remind executor/auditor agents to begin substantive reports with a compact Trace header and then explain in prose, and added `check_trace_protocol` to `scripts/dockit-validate-session.sh`. Durable enforcement is config-gated: `.dockit-config.yml` must declare `trace_protocol.enabled: true` and `trace_protocol.since: YYYY-MM-DD`; when enabled, HANDOFF needs `## Trace Anchor` and HISTORY entries dated >= since that reference backtick-quoted commit hashes need inline `Trace:` footers. `scripts/dockit-init-project.sh` now creates new projects with Trace enabled by default and a starter anchor/footer. `scripts/test-validator.sh` now covers 15 cases, including the Trace matrix. D-008 records the stable rationale for splitting chat orientation from durable validation. VERSION 4.8.2 -> 4.9.0; 8 doc-version markers in sync. DF-040 status -> `implemented (4.9.0)`. DF-035 option (b) remains the primary open candidate.
