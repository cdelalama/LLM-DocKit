...
- Cross-repo: tomatic gets `scripts/dockit-bootstrap-context.sh` + the matching SessionStart entry in `.claude/settings.json` applied directly in this session (not via `dockit-sync`) so the failure mode is closed for the project under active work; tomatic HANDOFF/HISTORY updated. Other downstream projects (`plaud-mirror`, `home-infra-protocol`, `infra-portal`, `llm-council`) close on next operator-driven `dockit-sync` pass.
