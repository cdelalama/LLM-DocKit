...
- Session Focus: **Cut v4.9.5 to refine Trace Protocol v1.3 after same-minute and stale-on-arrival evidence.** The chat `Sent` field now requires seconds on both sides (`YYYY-MM-DD HH:MM:SS <local-tz> (HH:MM:SS UTC)`), and the SessionStart payload tells receivers to re-check `git status`, `git log -1`, and the current clock before acting on an older Trace block's `Repo state`. DF-044 is filed and implemented. This is a patch to chat-side onboarding and documentation only; durable HANDOFF/HISTORY Trace validation is unchanged. Important carry-forward from the prior rollout remains: most synced downstream worktrees still contain uncommitted sync results; close those per repo before mixing in product work. DF-035 option (b) remains the primary DocKit design candidate after the rollout cleanup.
