...
**Likely next LLM-DocKit follow-ups:** retry the clean-adopter v4.12.1 sync wave that was blocked by DF-049, starting with `med`, `forgeos`, `home-infra-protocol`, and `youtube2text`. Keep `llm-council` and `plaud-mirror` separate because their dry-runs showed `LLM_START_HERE.md` section conflicts. Open a fresh Codex CLI session to verify DF-037 after the `--human` hook is installed. If the onboarding marker still repeats after turn 1, design a stateful Codex-specific output mode from observed behavior. No consensus/runtime ownership work remains in LLM-DocKit after D-011.
...
## Current Status
- Last Updated: 2026-06-20 - Codex GPT-5
- Session Focus: **Cut v4.12.1 closing DF-049 after MED/ForgeOS exposed copied doc-version rollback during v4.12.0 rollout.** `scripts/dockit-sync.sh` now normalizes copy-strategy HTML doc-version markers to the downstream project version before post-sync validation; D-014 records the rule and `scripts/test-validator.sh` covers the regression.
