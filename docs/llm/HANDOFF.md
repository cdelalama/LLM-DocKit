...
- Session Focus: **Cut v4.9.4 (patch) fixing section-merge new-section propagation before downstream sync.** While attempting to sync all adopters after v4.9.3, `scripts/dockit-sync.sh --dry-run` failed on full adopters whose `LLM_START_HERE.md` lacked the newly-added `trace-protocol` markers. DF-043 changes `section-merge`: when both markers for a template section are absent and `adoption_mode: full`, the sync inserts the full template block before the footer marker (or EOF if no footer) and records the section hash; malformed half-present markers still fail, and partial adopters still skip missing sections. Dry-runs that previously failed for `devenv`, `plaud-mirror`, and `nas-backup` now pass. VERSION 4.9.3 -> 4.9.4; 8 doc-version markers in sync. DF-043 status -> `implemented (4.9.4)`. Downstream sync is now the active execution gate.
