...
- Session Focus: **Cut v4.9.4 (patch) fixing section-merge new-section propagation, then rolled out sync broadly.** While attempting to sync all adopters after v4.9.3, `scripts/dockit-sync.sh --dry-run` failed on full adopters whose `LLM_START_HERE.md` lacked the newly-added `trace-protocol` markers. DF-043 changes `section-merge`: when both markers for a template section are absent and `adoption_mode: full`, the sync inserts the full template block before the footer marker (or EOF if no footer) and records the section hash; malformed half-present markers still fail, and partial adopters still skip missing sections. VERSION 4.9.3 -> 4.9.4; 8 doc-version markers in sync. DF-043 status -> `implemented (4.9.4)`. Rollout result: 24 projects now report `CURRENT` at template 4.9.4 and their bootstrap payloads emit Trace v1.2 `Sent: YYYY-MM-DD HH:MM <local-tz> (HH:MM UTC)`; `youtube2text` was prioritized after operator correction and synced successfully despite lacking a `VERSION` file (version post-sync validation skipped there by design). Remaining blockers: `camofox-browser` is OUTDATED but had a large dirty pre-existing worktree; `folio` is NO_STATE with a large untracked project tree; `claude-quest` and `house-thermal-monitor` are not registered because apply rolled back on project-local missing version markers; dirty unregistered repos (`audio-batch`, `msgvault-lab`, `rolodex`, `unifi-mcp`) were not touched. DF-035 option (b) remains the primary DocKit design candidate.
