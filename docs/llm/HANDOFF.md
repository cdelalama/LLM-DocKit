...
**Likely next LLM-DocKit follow-ups:** continue closing downstream dirty worktrees opportunistically when each adopter is opened. MED can sync v4.11.1, replace its local orientation-drift script by enabling the upstream check, and replace its local trace-anchor script by enabling durable Trace with `trace_protocol.reject_current_anchor_label: true` plus neutral `Subject:`/`Trace target:` anchor wording. No consensus/runtime ownership work remains in LLM-DocKit after D-011.
...
- Session Focus: **Cut v4.11.1 closing DF-048 from MED Trace feedback.** `trace_protocol.reject_current_anchor_label: true` now lets projects reject HANDOFF Trace Anchor labels that imply live currency for committed repo-side anchors. `scripts/dockit-trace-status.sh` generates close-out Trace fields from current git/date state to reduce stale chat Trace reports.
