...
**Likely next LLM-DocKit follow-ups:** open a fresh Codex CLI session to verify DF-037 after the `--human` hook is installed. If the onboarding marker still repeats after turn 1, design a stateful Codex-specific output mode from observed behavior. Continue closing downstream dirty worktrees opportunistically when each adopter is opened. No consensus/runtime ownership work remains in LLM-DocKit after D-011.
...
- Session Focus: **Cut v4.12.0 closing the Codex CLI integration axis from DF-036/DF-038.** LLM-DocKit now ships `scripts/dockit-install-codex-hook.sh`, documents Codex setup in `docs/integrations/CODEX.md`, records D-013, and keeps `docs/ROADMAP.md` as the source-only planning surface for semantic-check candidates and out-of-scope runtime boundaries.
