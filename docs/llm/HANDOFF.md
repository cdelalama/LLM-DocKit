...
**Likely next LLM-DocKit follow-ups:** continue closing downstream dirty worktrees opportunistically when each adopter is opened. MED can sync v4.11.0 and replace its local orientation-drift script by enabling the upstream check. The MED trace-anchor script functionality is already covered by DocKit Trace durable validation when `trace_protocol.enabled: true`. No consensus/runtime ownership work remains in LLM-DocKit after D-011.
...
- Session Focus: **Cut v4.11.0 closing DF-047 from MED orientation-drift feedback.** `scripts/dockit-validate-session.sh` now includes an opt-in `orientation-drift` check for phase-roadmap adopters. It parses completed phases from the configured roadmap and fails when configured entry docs still call a completed phase "next". The check skips by default so non-roadmap adopters do not get new failures. D-012 records the opt-in semantic-check policy.
