# Historical snapshot before DF-058 implementation

Not current instructions. Superseded by docs/llm/HANDOFF.md.
...
**2026-09-19 operator-corrected roadmap:** `docs/ROADMAP.md` replaces the
insufficient documentation-first plan. The first accepted delivery must include
prerequisite checks at a real project mutation entrypoint, durable retry/review
continuity, recovery and a real usable-outcome pilot. Documentation clarity is
supporting work, not the release's stopping point. `docs/llm/REVIEWS.md` preserves
the superseded agreement and exact Opus review of the corrected scope. Next:
specify the minimal contract and reproduce the late-ingress/equivalent-retry
failures in `scripts/test-validator.sh`; then implement the complete sequence.
No implementation, adopter mutation or release approval exists yet.
...
1. **Closed: all evidenced downstream adopters registered and updated** -
   ...
   adopters. All 38 carry `.dockit-enabled`. Thirty-seven active primary
   worktrees report the selected policy current; MED's protected active
   checkout lacks the section and its marker pair, while a separate policy
   commit is published on `origin/main`. Thirty-five policy revisions are
   published, `claude-quest`
   is a local-only third-party overlay, and dirty no-remote `cambio-claro` plus
   `juiced` retain additive local changes. Fifteen legacy adoptions were newly
   registered, including Irrigation Portal and late-discovered PiHA-Deployer.
   D-022 records the registration boundary. Full template versions were not
   advanced by this selective rollout. PiHA-Deployer retained its evidenced
...
1. **Closed: v4.15.0 audit remediation and source publication** - Exact Fable remained at its
   ...
   LOW and two NOTE issues. The executor agreed with every supported finding:
   the receipt's Travel Ledger diagnosis was false; the central validator had
   a separate UTC conversion bug; MED, PiHA-Deployer and protected-worktree
   wording overstated the evidence; legacy registration lacked an executable
   safe path; the audit itself was not recorded; two no-bump cases and an
   unmerged branch needed rationale; and the history/current-status chain was
...
hook reinstall and Home Infra Stop-gate rollout remain separate unrelated gates.
...
- Repository: `/home/cdelalama/src/LLM-DocKit`
- Branch and commit: local `main`, tracking `origin/main`, both at
  `b15b78f668569a8366ed49880a9c97c002b74ccd` before this documentation commit.
- Published version: `VERSION` = 4.13.1.
- Git state before this checkpoint: clean worktree, no stash entries, one
  worktree, remote branch parity verified, and `git fsck --full` PASS.
- Three project tmux panes were inspected read-only:
  `dev_admin_LLM-DocKit` was idle; `dev_gpt_LLM-DocKit` contained the completed
  v4.13.1 installer patch; `dev_claude_LLM-DocKit` contained the DF-054 audit
  and explicit operator authorization but no edits.
- No files exist under `.git/.dockit/`; no session baseline is needed for
  recovery. A fresh SessionStart will create its own baseline.
- The real `/home/cdelalama/.codex/config.toml` still contains the old
...
- No other repository was edited during this shutdown checkpoint.
- The many non-DocKit tmux panes were inventoried only by name/current path;
  their project state was not audited here and must be recovered from each
  repository's own HANDOFF.
- LLM-DocKit itself has no runtime dependency on the NAS. GitHub parity and
  adopter syncs do require restored network access; home-infra validation may
  additionally require its own services.
...
- Planning update: the operator rejected the documentation-only first slice.
  `docs/ROADMAP.md` now requires the complete delivery-control loop and a real
  integrated pilot; `docs/llm/REVIEWS.md` records the replacement Opus agreement.
  Implementation has not begun; behavior remains 4.15.0.
...
- Session Focus: **Published v4.15.0 as the durable closeout of the authorized
  ...
  protected dirty no-remote exceptions, and 11 explicitly non-registered
  primary repositories. D-022 and `HOW_TO_USE.md` make registration explicit
  and distinguish it from selected-policy delivery and full-template currency;
  the sync tool can now create that honest state directly. The Trace validator
  converts offset commit times to UTC and the smoke suite covers both changes.
