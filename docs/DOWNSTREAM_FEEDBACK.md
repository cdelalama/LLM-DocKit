...
- Category: gap, drift
- Status: partially implemented (4.8.0) — option (a) static `check_template_residue` shipped in `scripts/dockit-validate-session.sh`. Greps `LLM_START_HERE.md` / `docs/STRUCTURE.md` / `docs/ARCHITECTURE.md` for the literal residue patterns catalogued in this DF (`Replace angle-bracket placeholders`, `Customization Notes for Maintainers`, `Replace <project> with the actual project name`, `Use this template to document`, `<PROJECT_ROOT>`, `<Names>`, `<Invariant`, `<Step>`, `<Phase 0>`, `Authors: <Names>`); FAIL on any match. `docs/llm/DECISIONS.md` empty-after-threshold check WARNs (not FAIL) once the repo crosses `DOCKIT_DECISIONS_EMPTY_THRESHOLD_COMMITS` commits (default 5). Skips on the LLM-DocKit source repo itself (presence of `dockit-sync-manifest.yml` is the source-repo marker). Option (b) — strip-at-scaffold-time inside `dockit-init-project.sh` — remains `open`; the (b.i) delete `docs/ARCHITECTURE.md` from default vs (b.ii) rename to `.example` decision is design, not mechanical, and merits its own deliberation pass after this minor's smoke sweep produces empirical data on adopter residue rates. Option (c) mandatory DECISIONS gate also remains `open` for v5 or later. The 4.8.0 smoke test of `dockit-init-project.sh` confirmed empirically that a freshly-scaffolded project FAILS the new check — that is the correct signal until (b) lands.
