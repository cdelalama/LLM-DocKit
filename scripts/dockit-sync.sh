...
#                      With --init-state, preserve unknown prior template identity
...
RESTORE_TS=""
ONLY_SELECTORS=""
UNTRACKED_EXISTING_ADOPTION=false
...
write_state() {
    ...
    _state_template_version="$TEMPLATE_VERSION"
    _state_template_ref="$TEMPLATE_REF"
    if [ "$_mode" = "init-state" ] && $UNTRACKED_EXISTING_ADOPTION; then
        _state_template_version="pre-registration"
        _state_template_ref="untracked-existing-adoption"
    fi

    _state_tmp="$TMPDIR/state_new.yml"
    cat > "$_state_tmp" <<STATEEOF
# .git/.dockit/state.yml -- auto-generated by dockit-sync.sh. Do not edit.
template_version: "$_state_template_version"
template_ref: "$_state_template_ref"
last_sync_at: "$_now"
...
do_init_state() {
    ...
    _target_ref="$TEMPLATE_REF"
    if $UNTRACKED_EXISTING_ADOPTION; then
        _target_version="pre-registration"
        _target_ref="untracked-existing-adoption"
    fi

    if [ -f "$_state_file" ]; then
        _existing_version=$(read_state_version "$_state_file")
        _existing_ref=$(read_state_ref "$_state_file")
        if { [ "$_existing_version" = "pre-registration" ] || \
             [ "$_target_version" = "pre-registration" ]; } \
           && { [ "$_existing_version" != "$_target_version" ] || \
                [ "$_existing_ref" != "$_target_ref" ]; }; then
            _identity_change="$_existing_version / $_existing_ref -> $_target_version / $_target_ref"
            if ! $FORCE; then
                warn "Refusing to replace sync identity $_identity_change; rerun with --force only after verifying provenance"
                return 1
            fi
            warn "Replacing sync identity $_identity_change because --force was supplied"
        fi
    fi
    ...
    if $UNTRACKED_EXISTING_ADOPTION; then
        info "Template identity recorded as pre-registration / untracked-existing-adoption."
    fi
...
sync_project() {
    ...
    if [ "$MODE" = "init-state" ]; then
        if ! do_init_state "$_project_root"; then
            report_entry "__project__" "ERROR" "state initialization refused"
            release_lock
            return 1
        fi
        release_lock
...
parse_args() {
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --init-state)
                ...
                ;;
            --untracked-existing-adoption)
                UNTRACKED_EXISTING_ADOPTION=true
                ;;
            --restore)
                MODE="restore"
                shift
    ...
    if $UNTRACKED_EXISTING_ADOPTION; then
        if [ "$MODE" != "init-state" ]; then
            die "--untracked-existing-adoption requires --init-state"
        fi
        if [ -z "$PROJECT_PATH" ] || $SYNC_ALL; then
            die "--untracked-existing-adoption requires one --project, not --all"
        fi
    fi
}
